...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
            ...
            type=str,
            default="realsr",
            choices=['realsr', 'bicsr', 'inpaint_imagenet', 'inpaint_face', 'faceir', 'deblur', 'su-resshift', 'su-resshift+ef'],
            help="Chopping forward.",
            )
...
def get_configs(args):
    ...
    # su-resshift    2025-06-16-21-24
    elif args.task == 'su-resshift':
        configs = OmegaConf.load('./configs/sea_undistort_resshift.yaml')
        assert args.scale == 1, 'Please set scale equals 1 for deblurring!'
        ckpt_url = _LINK[args.task]
        ckpt_path = ckpt_dir / f'su_resshift_200000.pth'
        vqgan_url = _LINK['vqgan']
        vqgan_path = ckpt_dir / f'autoencoder_vq_f4.pth'
    # su-resshift+ef2   2025-07-12-18-33
    elif args.task == 'su-resshift+ef':
        configs = OmegaConf.load('./configs/sea_undistort_resshift_ef.yaml')
        assert args.scale == 1, 'Please set scale equals 1 for deblurring!'
        ckpt_url = _LINK[args.task]
        ckpt_path = ckpt_dir / f'su_resshift+ef_160000.pth'
        vqgan_url = _LINK['vqgan']
        vqgan_path = ckpt_dir / f'autoencoder_vq_f4.pth'
...
def main():
    ...
    elif args.task.startswith('su-resshift+ef'):
        assert args.mask_path, 'Please input the mask path for inpainting!'
        mask_path = args.mask_path
    else:
        mask_path = None
