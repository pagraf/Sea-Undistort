...
class ImageSpliterTh:
    def __init__(self, im, pch_size, stride, sf=1, extra_bs=1):
        ...
        self.im_ori = im
        self.im_res = torch.zeros([bs, 3, height*sf, width*sf], dtype=im.dtype, device=im.device) # self.im_res = torch.zeros([bs, chn, height*sf, width*sf], dtype=im.dtype, device=im.device)
        self.pixel_count = torch.zeros([bs, 3, height*sf, width*sf], dtype=im.dtype, device=im.device) # self.pixel_count = torch.zeros([bs, chn, height*sf, width*sf], dtype=im.dtype, device=im.device)
