...
class ResShiftSampler(BaseSampler):
    def sample_func(self, y0, noise_repeat=False, mask=False):
        ...
        if not (ori_h % offset == 0 and ori_w % offset == 0):
            ...
            # Falls eine Maske vorliegt, ebenfalls paddieren
            if mask is not None:
                mask = F.pad(mask, pad=(0, pad_w, 0, pad_h), mode='reflect')
        else:
            flag_pad = False
    ...
    def inference(self, in_path, out_path, mask_path=None, mask_back=False, bs=1, noise_repeat=False):
        '''
        Inference demo.
        Input:
            ...
            if im_lq_tensor.shape[2] > self.chop_size or im_lq_tensor.shape[3] > self.chop_size:
                ...
                for im_lq_pch, index_infos in im_spliter:
                    ...
                    if mask is not None:
                        ...
                        # print(im_lq_pch.shape, mask_pch.shape)
                    else:
                        mask_pch = None
                    with context():
        ...
        if in_path.is_dir():
            if mask_path is None:
                data_config = {'type': 'base',
                               'params': {'dir_path': str(in_path),
                                          ...
                                          'need_path': True,
                                          'recursive': True,
                                          'im_exts': ['png', 'jpg', 'jpeg', 'JPEG', 'bmp', 'PNG', 'JPG', 'tif'],
                                          'length': None,
                                          }
                               }
            else:
            ...
            for data in dataloader:
                ...
                # print(micro_data['lq'].shape, micro_data['mask'].shape)
                # print(micro_data['lq'].min(), micro_data['lq'].max(), micro_data['mask'].min(), micro_data['mask'].max())

                if micro_data['lq'].shape[0] > 0:
                    results = _process_per_image(
                            micro_data['lq'].cuda() * 0.5 + 0.5 if non_negative else micro_data['lq'].cuda(),
            ...
            # print(im_mask.min(), im_mask.max(), im_mask.mean(), im_mask.shape)

            im_sr_tensor = _process_per_image(
                    im_lq_tensor if non_negative else (im_lq_tensor - 0.5) / 0.5,
                    mask=(im_mask_tensor - 0.5) / 0.5 if mask_path is not None else None,
                    )

            print(im_sr_tensor.shape, im_sr_tensor.min(), im_sr_tensor.max())
