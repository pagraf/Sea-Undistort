...
class TrainerBase:
    ...
    def build_dataloader(self):
        ...
        bathy = True
        if bathy:
            datasets = {'train': SeaUndistort(self.configs.data.get('train', dict).params, split_mode='train'), }
            if hasattr(self.configs.data, 'val') and self.rank == 0:
                datasets['val'] = SeaUndistort(self.configs.data.get('val', dict).params, split_mode='val')
        else:
    ...
    def train(self):
        ...
        for ii in range(self.iters_start, self.configs.train.iterations):
            ...
            if ii % 10 == 0:
                print("Current Iteration: ", self.current_iters)
